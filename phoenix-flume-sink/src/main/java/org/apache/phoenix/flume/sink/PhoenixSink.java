...
public final class PhoenixSink extends AbstractSink implements Configurable {
    ...
    private void initializeSerializer(final Context context, final String eventSerializerType) {
        ...
        try {
            ...
            logger.error("An invalid eventSerializer {} was passed. Please specify one of {} ", eventSerializerType, Joiner.on(",").skipNulls().join(EventSerializers.values()));
    ...
    public Status process() throws EventDeliveryException {
        ...
        try {
            ...
            logger.debug(getName() + " Time taken to process [" + events.size() + "] events was [" + (System.currentTimeMillis() - startTime) + "] ms");
            if (transaction != null) {
                transaction.close();
            }
        }
